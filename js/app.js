...
function getImgs() {
    ...
    myRequest.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            const imgsSrc = JSON.parse(this.responseText);
            function randomImg() {
                durationInput.value = 5;
                if (backgroundOption === true) {
                    interval = setInterval(() => {
                        let randomNum = Math.floor(Math.random() * imgsSrc.length);
                        landingPage.style.background = "url(/images/" + imgsSrc[randomNum].src + ") center/cover";
